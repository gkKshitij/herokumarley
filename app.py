    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            data={
                ...
                "gray_img":'static/downloads/gray'+filename,
                "blurred_img":'static/downloads/blurred'+filename,
                "edged_img":'static/downloads/edged'+filename,
                "dilated_img":'static/downloads/dilated'+filename,
                "eroded_img":'static/downloads/eroded'+filename,
                "thresh_img":'static/downloads/thresh'+filename,
                "mask_img":'static/downloads/mask'+filename,
                "processed_img":'static/downloads/processed'+filename
                }
...
##
    ...
    cv2.imwrite(f"{DOWNLOAD_FOLDER}gray/{filename}",gray)
    
    blurred = cv2.GaussianBlur(gray, (11, 11), 0)
    cv2.imwrite(f"{DOWNLOAD_FOLDER}blurred/{filename}",blurred)
    edged = cv2.Canny(blurred, 50, 150)
    cv2.imwrite(f"{DOWNLOAD_FOLDER}edged/{filename}",edged)
    dilated = cv2.dilate(edged, None, iterations=4) # 10 is an interesting number
    cv2.imwrite(f"{DOWNLOAD_FOLDER}dilated/{filename}",dilated)
    eroded = cv2.erode(dilated, None, iterations=4)
    cv2.imwrite(f"{DOWNLOAD_FOLDER}eroded/{filename}",eroded)
    thresh = cv2.threshold(eroded, 225, 255, cv2.THRESH_BINARY_INV)[1]
    cv2.imwrite(f"{DOWNLOAD_FOLDER}thresh/{filename}",thresh)

    mask = thresh.copy()
    mask = cv2.erode(mask, None, iterations=1)
    cv2.imwrite(f"{DOWNLOAD_FOLDER}mask/{filename}",mask)
    ...
    cv2.imwrite(f"{DOWNLOAD_FOLDER}processed/{filename}",processed)
